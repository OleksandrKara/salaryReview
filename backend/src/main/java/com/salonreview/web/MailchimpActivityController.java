 ...
 *
 * <p>Also naturally includes any pure-email one-off campaign (e.g. {@code color_booster_winback_oneoff},
 * see {@code ColorBoosterWinbackOneOffService}) — that automation logs into this same table (just
 * with a null {@code sms_message_id}, since it has no SMS leg), so nothing special is needed here
 * to surface it alongside the SMS-fallback automations.
 */
@RestController
