 ...
 * {@code ai.funnel-analysis.enabled}, or {@code ai.sms-draft.enabled} is on (all call Claude via
 * the same {@code ANTHROPIC_API_KEY}); when all are off there is literally no AI code to
 ...
 */
@Configuration
@ConditionalOnExpression("${ai.triage.enabled:false} or ${rag.enabled:false} or ${ai.funnel-analysis.enabled:false} or ${ai.sms-draft.enabled:false}")
        ...
        if (!props.isAnthropicConfigured()) {
            throw new IllegalStateException(
                    "An AI feature is enabled (ai.triage.enabled, rag.enabled, ai.funnel-analysis.enabled, "
                            + "or ai.sms-draft.enabled) but ANTHROPIC_API_KEY is missing. Either set the key "
                            + "(see .env.example) or disable the feature.");
        }
