...
- [ ] 3.1 Add `com.google.apis:google-api-services-searchconsole` / equivalent GA4 Data API client / plain `RestClient` for PageSpeed Insights to `pom.xml`
- [ ] 3.2 `SearchConsoleClient` — `searchAnalyticsQuery(SeoConnection, LocalDate)`, mirrors `SquareClient`'s per-instance-per-business construction pattern (design.md D4 — one client instance per business, not a shared singleton)
- [ ] 3.3 `GoogleAnalyticsClient` — `runReport(SeoConnection, DateRange)`
- [ ] 3.4 `PageSpeedInsightsClient` — `check(url, strategy, apiKey)`
