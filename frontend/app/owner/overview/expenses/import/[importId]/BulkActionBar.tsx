...
export default function BulkActionBar({
  count, sameMerchant, merchantName, onApply, onClear, busy,
}: {
  count: number;
  sameMerchant: boolean;
  merchantName?: string;
  onApply: (selection: CategorySelection, remember: boolean) => Promise<void> | void;
  onClear: () => void;
  busy: boolean;
}) {
  ...
  return (
    <div className="sticky bottom-0 z-10 mt-2 flex flex-wrap items-center gap-2 rounded-lg bg-zinc-800 px-3 py-2 text-white shadow-lg">
      ...
      <button
        ...
        onClick={() => onApply(selection, sameMerchant && remember)}
