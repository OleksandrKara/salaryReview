...
// month/weekday names below switch locale with the UI language (ru-RU when RU is selected, same
// lang==='RU'?'ru-RU':'en-US' convention as my-documents/page.tsx and SopList.tsx) — everything
// else in this file already goes through t(), but these are generated by Intl, not the string
// catalog, so they'd otherwise stay in English even with RU selected. Currency stays en-US
// regardless (usd() below) — that's this whole codebase's existing convention, not a gap: this is
// a US salon pricing in USD, and every other page's amounts format the same way.
function formatDate(iso: string, lang: Language | null): string {
  ...
  return new Date(y, m - 1, d).toLocaleDateString(lang === 'RU' ? 'ru-RU' : 'en-US', { month: 'short', day: 'numeric', year: 'numeric' });
}

function formatTime(hhmm: string, lang: Language | null): string {
  // Intl's own AM/PM-vs-24-hour default per locale (ru-RU renders 24-hour, e.g. "18:00", en-US
  // renders 12-hour with AM/PM) rather than a hand-rolled always-12-hour formatter — matches how a
  // Russian reader actually expects to see a time, not just translated month/weekday names.
  const [h, m] = hhmm.split(':').map(Number);
  return new Date(2000, 0, 1, h, m).toLocaleTimeString(lang === 'RU' ? 'ru-RU' : 'en-US', { hour: 'numeric', minute: '2-digit' });
}
...
function monthLabel(key: string, lang: Language | null): string {
              ...
              <div className="overflow-hidden rounded-xl ring-1 ring-zinc-200">
                <table className="w-full text-sm">
                  <tbody className="divide-y divide-zinc-100">
                    {byMonth.map(([key, v]) => (
                      <tr key={key}>
                        <td className="px-3 py-1.5 text-zinc-600">{monthLabel(key, language)}</td>
            ...
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="font-medium text-zinc-800">
                  {formatDate(r.requestedDate, language)}
                  {r.requestedTime && <span className="text-zinc-500"> · {formatTime(r.requestedTime, language)}</span>}
                </p>
          ...
          <tbody className="divide-y divide-zinc-100">
            {rows.map((r) => (
              <tr key={r.id} className="hover:bg-zinc-50">
                <td className="px-3 py-2 tabular-nums text-zinc-600">{formatDate(r.requestedDate, language)}</td>
                <td className="px-3 py-2 tabular-nums text-zinc-500">{r.requestedTime ? formatTime(r.requestedTime, language) : '—'}</td>
