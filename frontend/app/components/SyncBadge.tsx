...
export function SyncBadge({ syncedAt, timezone, language = null }:
  { syncedAt: string; timezone?: string; language?: Language | null }) {
  const d = new Date(syncedAt);
  const time = isNaN(d.getTime())
    ? ''
    : d.toLocaleString(language === 'RU' ? 'ru-RU' : 'en-US', {
  ...
  return (
    <span className="inline-flex items-center gap-2">
      ...
      >
        ...
        {t(language, 'syncSynced')}{time && <span className="font-normal text-emerald-600">· {time}</span>}
      </span>
